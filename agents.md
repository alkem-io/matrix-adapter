...
# agents.md v2.0 (Go Implementation)
...
1. Compare base branch (default: `main`, override if branched from release).
2. Group changed files by category: `internal/core`, `internal/infrastructure`, `pkg/dto`, `cmd`, `tests`.
3. Summarize semantic changes: new DTOs, events, domain models, migrations.
...
internal/core 3 Added domain/invite_logic.go
pkg/dto 1 Added new command payload
tests 7 Coverage for invite logic
Contract Deltas

Commands: +1 command (InviteUser), 0 removals
Events: +room.invite.received
Lib Version: MINOR bump required (generated)

Risk & Mitigations

✅ Mitigated: race condition on invite (p95 <120 ms)

⚠️ Remaining: federation lag (tracked separately)

Outcomes vs Target

Goal: reliable user invites
...
| ---------- | -------------------- | ----------- | -------------------------- |
| 2025-11-25 | v2.0.0 Go Adaptation | (add on PR) | Adapted for Go architecture |
