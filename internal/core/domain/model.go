...
// Attachment carries the document to send or the media reference from an event.
// A received DocumentID is a user-writable hint, not an authorization grant.
type Attachment struct {
	DocumentID  string
	MediaID     string
	DisplayName string
	MimeType    string
	// Outbound uploads use file-service Content-Length instead of this hint.
	Size   int64
	Width  *int
	Height *int
}
