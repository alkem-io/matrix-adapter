...
func (h *AdminHandler) HandleGetAllRooms(_ []byte) (interface{}, error) {
	...
	for _, r := range rooms {
		roomResponses = append(roomResponses, dto.RoomDetailsResponse{
			RoomID: r.ID.String(),
			Name:   r.Name,
			Topic:  r.Topic,
			Alias:  r.Alias,
...
func (h *AdminHandler) HandleReplicateRoomMembership(payload []byte) (interface{}, error) {
	...
	added, failed, err := h.service.ReplicateRoomMembership(context.Background(), id.RoomID(req.SourceRoomID), id.RoomID(req.TargetRoomID), domain.Actor{ID: prioritizerID})
	if err != nil {
		return nil, err
	}
