...
The `pkg/dto` package in the Go service defines the canonical command payloads, event types, and DTO structures. The shared TypeScript library (`@alkemio/matrix-adapter-lib`) MUST be generated from these Go definitions (e.g., using `cmd/gen-events` or `tygo`). Manual edits to the TypeScript definitions are forbidden.
